					...
					<form method="POST" action="{{ route('post.update',$post->id) }}" enctype="multipart/form-data">
						...
						{{method_field('PUT')}}
							<div class="row">
								<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
									<div class="form-group">				
										...
										<input type="text" name="title" id="title" class="form-control" title="title" value="{{$post->title}}">
									</div>
									<div class="form-group">				
									...
									<div class="form-group">
										<input type="checkbox" name="is_active" id="is_active" value="{{$post->is_active}}" {{$post->is_active==true? 'checked' : ''}}>><i></i>Status
									</div>									
								</div>
                            ...
                            @foreach($post->categories as $postcategory)
                                {{$postcategory->id == $row->id ? 'selected' : ''}}
                            @endforeach
                            ...
                            value="{{$row->id}}">{{$row->name}}
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<textarea name="body" class="form-control my-editor" rows="15">
										{{$post->body}}
									</textarea>
								</div>
							</div>
